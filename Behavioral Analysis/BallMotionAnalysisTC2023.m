%Code to Determine Speed & Direction from Ball Motion Videos
%Developed by Gunvant Chaudhari, 2017; Modified for 50 fps videos by Trish
%Chari
...
rd_dir_f = 'F:\Trish\Running Avoidance Analysis\Videos for Social Touch';
...
% Take 5 frames (0.1s)
% 1. consolidate 5 frames' data into a list
...
%NOTE THIS IS HARD CODED FOR 5 FRAME MEDIANS, DONT FORGET THAT
% trialSpeed = zeros(60,num_trials);
% for eachT = 2:size(gunvantFrames,1)
%     thisF = gunvantFrames(eachT);
%    
%     
%     startMed = ((thisF-START_FRAME)/5)-(10*3); %Factoring in for 3seconds before stimulation
%     weight = (mod((thisF-START_FRAME),5))/5;
%     
%     if startMed>0
%         for i = 0:59
...
% open(writerObj); 
% for frame = START_FRAME:(END_FRAME-5)
%     
%     bw = read(ball_motion_vid,frame);
%     imshow(bw)
%     
...
%     
%     index = floor((frame-START_FRAME)/5)+1;
